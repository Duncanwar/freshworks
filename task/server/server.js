...
onTicketCreateCallback: async function(args) {
    ...
    try{
const ticket = await createTicket(iparams,data)
    }catch(err){
        console.error(err)
    }
            },
...
async function createTicket(iparams,data) {
    ...
    // };
    try {
        const fd_ticket= await $request.post(`https://${iparams.domain_url}/api/v2/tickets`,{
            headers: {
        ...
        const fs_ticket = await $request.post(`https://${iparams.fs_domain_url}/api/v2/tickets`,{
            headers: {
                Authorization: `Basic ${fs_base64Encoded}`,
                "Content-Type": "application/json"
              }, 
...
console.log(freshservice_ticket_id)

        const add_fs_ticke_id =  await $request.put(`https://${iparams.domain_url}/api/v2/tickets/${freshdesk_ticket_id}`,{
            headers: {
                Authorization: `Basic ${base64Encoded}`,
                "Content-Type": "application/json"
              }, 
